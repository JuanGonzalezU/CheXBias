...
for experiment in ['Experiment_2']:
    ...
    for group in ['age']:#groups:
        ...
        for name_model in models:
            ...
            # Load weights
            model.load_state_dict(torch.load(os.path.join(path_model,name_model,'best_model.pth'),map_location=torch.device('cpu')))    
            ...
            with torch.inference_mode():
                ...
                if group == 'sex':                  
                    ...
                    for batch,(file_names,X,y) in tqdm(enumerate(data_loader_test)):
                        ...
                        #  Encode files by age
                        encoded_age = [int(file_name.split('_')[5]) for file_name in file_names]  
