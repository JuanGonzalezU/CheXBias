...
class ChexpertSmall(Dataset):
    url = 'http://download.cs.stanford.edu/deep/CheXpert-v1.0.zip'
    ...
    def __init__(self, root, mode='train', transform=None, data_filter=None, mini_data=None):
        ...
        if mode == 'test':
            ...
            print('Dataset loaded. Length: ', len(self.data))
    ...
    def _maybe_process(self, data_filter):
        ...
        if not (os.path.exists(train_file) and os.path.exists(valid_file)):
            ...
            train_df = self._load_and_preprocess_training_data(os.path.join(self.root, self.dir_name, 'train_VisualCheXbert.csv'), data_filter)
