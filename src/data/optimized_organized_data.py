...
for dir in tqdm(os.listdir(join_path(raw_data_path, 'train'))):
    for study in os.listdir(join_path(raw_data_path, 'train', dir)):
        for image in os.listdir(join_path(raw_data_path, 'train', dir, study)):            
            img_name, ext = os.path.splitext(image)
            new_name = f'{dir}_{study}_{img_name}_'

            # Look up the data from the dictionary
            df_row = train_dict.get(join_path('CheXpert-v1.0','train', dir, study, image))
            #breakpoint()
            ...
            if df_row:
                ...
                new_file_path = join_path(data_path, 'interim','train', new_name)
                
                # Copy file if it hasn't been processed before
                if new_file_path not in processed_files:
                    shutil.copyfile(join_path(raw_data_path, 'train', dir, study, image), new_file_path)
